...
*/
        ...
        private int GetPokemonStat(int pokemonIndex, string statType)
        {
            ...
            if (!Stats.ContainsKey(statType.ToUpperInvariant()))
            {
                Fatal("error: getPokemonStat: the stat '" + statType + "' does not exist.");
                return 0;
            }

            return Bot.Game.Team[pokemonIndex - 1].Stats.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private int GetPokemonEffortValue(int pokemonIndex, string statType)
        {
            ...
            return Bot.Game.Team[pokemonIndex - 1].Ev.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private int GetPokemonIndividualValue(int pokemonIndex, string statType)
        {
            ...
            return Bot.Game.Team[pokemonIndex - 1].Iv.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private static readonly Dictionary<string, StatType> Stats = new Dictionary<string, StatType>
        {
            {"HP", StatType.Health},
        ...
        private bool IsOpponentEffortValue(string statType)
        {
            ...
            return stats.HasOnly(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private int GetOpponentEffortValue(string statType)
        {
            ...
            var stats = EffortValuesManager.Instance.BattleValues[Bot.Game.ActiveBattle.OpponentId];
            return stats.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private int GetPokemonStatFromPc(int boxId, int boxPokemonId, string statType)
        {
            ...
            return Bot.Game.CurrentPcBox[boxPokemonId - 1].Stats.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private int GetPokemonEffortValueFromPc(int boxId, int boxPokemonId, string statType)
        {
            ...
            return Bot.Game.CurrentPcBox[boxPokemonId - 1].Ev.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private int GetPokemonIndividualValueFromPc(int boxId, int boxPokemonId, string statType)
        {
            ...
            return Bot.Game.CurrentPcBox[boxPokemonId - 1].Iv.GetStat(Stats[statType.ToUpperInvariant()]);
        }
        ...
        private static readonly HashSet<int> OutOfCombatItemScopes = new HashSet<int> {8, 10, 15};
        private static readonly HashSet<int> InCombatItemScopes = new HashSet<int> {5};
        private static readonly HashSet<int> OutOfCombatOnPokemonItemScopes = new HashSet<int> {2, 3, 9, 13, 14};
        private static readonly HashSet<int> InCombatOnPokemonItemScopes = new HashSet<int> {2};
        ...
        private bool UseItem(string itemName)
        {
            ...
            if (item != null && item.Quantity > 0)
                if (Bot.Game.IsInBattle && InCombatItemScopes.Contains(item.Scope))
                {
                    if (!ValidateAction("useItem", true)) return false;
                    return ExecuteAction(Bot.Ai.UseItem(item.Id));
                }
                else if (!Bot.Game.IsInBattle && OutOfCombatItemScopes.Contains(item.Scope))
                {
        ...
        private bool UseItemOnPokemon(string itemName, int pokemonIndex)
        {
            ...
            if (item != null && item.Quantity > 0)
                if (Bot.Game.IsInBattle && InCombatOnPokemonItemScopes.Contains(item.Scope))
                {
                    if (!ValidateAction("useItemOnPokemon", true)) return false;
                    return ExecuteAction(Bot.Ai.UseItem(item.Id, pokemonIndex));
                }
                else if (!Bot.Game.IsInBattle && OutOfCombatOnPokemonItemScopes.Contains(item.Scope))
                {
